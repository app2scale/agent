...
ACTION_SPACE = Discrete(432) # index of the possible states
replica = [1, 2, 3]
cpu = [4, 5, 6, 7, 8, 9]
heap = [4, 5, 6, 7, 8, 9]
colling_down = [3, 4, 5]

POSSIBLE_STATES = np.array(list(product(replica, cpu, heap, colling_down)), dtype=np.float32)
...
class CustomLoad(LoadTestShape):
    ...
    trx_load_data = pd.read_csv("./server_client_v3/transactions.csv")
...
def collect_metrics(env, cooling_down_coeff):
    ...
    time.sleep(cooling_down_coeff*WARM_UP_PERIOD)
...
def step(action, state, env):
    ...
    temp_state = np.array(POSSIBLE_STATES[action],dtype=np.int32)
    temp_updated_state = np.array([temp_state[0], temp_state[1], temp_state[2]])

    print('applying the state...')
    print("updated state", temp_updated_state)
    update_and_deploy_deployment_specs(temp_updated_state)
    ...
    temp_previous = previous_tps
    metrics = collect_metrics(env, temp_state[3])

    instant_tps = metrics["num_requests"]
    previous_tps = instant_tps
    ...
    return new_state, reward, metrics, deployment_time, temp_previous, instant_tps
...
step_count = 1
sum_reward = 0
prev_state = np.append(prev_state,2) 
while True:
    ...
    state, reward, info, deployment_time, prev_tps, inst_tps = step(action, prev_state, env)
    ...
    temp_output = [state[0], state[1], state[2], prev_tps,
                   inst_tps, info["cpu_usage"], 
